...
import { useFundsData } from '../hooks/useFundsData'; // ה-Hook החדש שיצרנו
...
export function FundsManagement() {
  ...
  // שימוש ב-Hook הייעודי שמביא רק נתוני קרנות מהשרת
  const { 
    transactions, 
    loading, 
    pagination, 
  ...
  // טיפול בחיפוש מקומי (אופציונלי - אם השרת לא מסנן חיפוש בנתיב הקרנות)
  const filteredTransactions = transactions.filter(trx => 
    trx.workerName?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
    trx.fundNumber?.includes(debouncedSearch) ||
    trx.notes?.toLowerCase().includes(debouncedSearch.toLowerCase())
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 text-right" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="relative w-64">
          ...
          <input
            type="text"
            placeholder="חיפוש לפי עובדת/קרן/הערה..."
            className="w-full pr-9 pl-4 py-2 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm transition-all"
            value={inputValue}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400 italic">
                    לא נמצאו תרומות לקרנות התואמות לחיפוש
                  </td>
                </tr>
              ) : (
                filteredTransactions.map((trx) => (
                  <tr key={trx.id} className="hover:bg-slate-50/50 group transition-colors">
                    <td className="px-6 py-4">
                      <span className="bg-amber-50 text-amber-700 px-3 py-1 rounded-full font-black border border-amber-100 shadow-sm">
                        {trx.fundNumber || '---'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <div className="text-xs font-bold text-slate-500">
            {pagination ? (
              <span>מציג {filteredTransactions.length} מתוך {pagination.total} תרומות</span>
            ) : (
              <span>עמוד {page}</span>
            )}
          </div>
          <div className="flex gap-2">
            <button 
              ...
              className="p-2 bg-white border border-slate-200 rounded-lg disabled:opacity-50 hover:bg-slate-50 transition-colors shadow-sm"
            >
      ...
      <NewDonationModal
        ...
        onRefresh={() => { 
          fetchFunds(); // קריאה ל-fetch של הקרנות בלבד
          setIsModalOpen(false); 
        }}
        editingDonation={editingTransaction}
        branches={allBranches}
        // שליחת ה-ID הנכון למודאל
