...
export function BranchesManagement() {
  ...
  const [newBranchData, setNewBranchData] = useState<{ id: number; name: string } | null>(null);
  ...
  const handleSave = async (data: Partial<Branch>) => {
    try {
        const saved = await saveBranch(data, selectedBranch?.id);
        toast.success(selectedBranch ? 'הסניף עודכן בהצלחה' : 'סניף חדש נוסף בהצלחה');
        refreshBranches();
        setIsModalOpen(false);

        // רק בסניף חדש — פתח מודאל משתמש
        if (!selectedBranch && saved?.id) {
            setNewBranchData({ id: saved.id, name: data.name || '' });
  ...
  return (
            ...
            <tbody className="divide-y divide-slate-100">
              {allBranches.map((branch: Branch) => (
                <tr key={branch.id} className="hover:bg-slate-50 group transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${branch.is_active
    ...
    onClose={() => { setIsUserModalOpen(false); setNewBranchData(null); }}
    branchId={newBranchData?.id || 0}
