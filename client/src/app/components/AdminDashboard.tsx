          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right text-sm">
              <thead className="bg-slate-50/50 border-b border-slate-200 text-slate-500 font-bold">
                <tr>
                  <th className="px-6 py-4 text-xs">מזהה</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {transactions.map((trx) => (
                  <tr key={trx.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium">{trx.date}</td>
                    <td className="px-6 py-4 text-slate-600">{trx.branch}</td>
                    <td className="px-6 py-4 font-bold text-slate-900">₪{(trx.amount || 0).toLocaleString()}</td>
                    <td className="px-6 py-4 text-center">
                      <button onClick={() => openEditModal(trx)} className="p-2 hover:bg-indigo-50 text-indigo-600 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={onFormSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">סכום לתרומה (₪)</label>
                <input
                  type="number"
                  step="0.01"
                  required
                  ...
                  className="w-full px-4 py-3 text-2xl font-black text-indigo-900 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="0.00"
                />
              </div>
